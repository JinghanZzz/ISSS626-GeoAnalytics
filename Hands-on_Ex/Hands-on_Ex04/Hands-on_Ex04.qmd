...
-   compute spatial weights uundefinedsing appropriate functions of **spdep** package, and

-   calculate spatially lagged variables using appropriate functions of **spdep** package.

process：

1 先把attribute贴到spatial的data上
...
To check neibougher of 1, we see 2,3,4,57 and 85 are its neighbor
...
do the same for latitude. We access the second value per each centroid with \[\[2\]\].
...
```{r}
#show how many neighbours does it have
...
下面的output是object\[1\]的5个neighbour的反距离权重
...
**Equal Weight** (style = "W"): neighbours share the weight. Unfair when some places has fewer neighbours so each of their neighbours gain higher weight

**Binary Weight** (style ="B") : eighther is neighbour ==1, or not neighbour ==0
...
-   spatial lag with row-standardized weights, (equal weight)

-   spatial lag as a sum of neighbouring values, (sum target value, when you want to know the total influence of your neighbours)

-   spatial window average, and (avg of neighbours)
...
先assign weight==1 to neighbours
...
assign 完用lag.listw to compute a lag variable from out weight and GDPPC
...
find neighbour–—add diagonal element(就是自己和自己也是邻居，不仅考虑邻里关系，还要考虑自己对自己的关系，就是proximity matrix里，把表示自己和自己的对角线元素，从0改为1）–—assign weight
...
然后assgin weight，再create lag variable from weight structure ans GDPPC variable
...
这时候lag variable是个lisst， convert to a data frame by using data.frame
