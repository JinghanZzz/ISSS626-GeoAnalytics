...
就是第八章算完加权然后拿那个weight来算autocorrelation，就是看一个地方和它旁边位置相似，是不是attribute也相似
...
左边因为等间距，那分布本来就是正太分组，本来就是大家都集中在一个range左右，所以要拿quantile来分
...
一行就是一个人的全部neighbour，所以row standard就是他们share掉这个人的weight。 standardise就是分之一
...
首先得看一下 别是random的 random的后面就不用管了
