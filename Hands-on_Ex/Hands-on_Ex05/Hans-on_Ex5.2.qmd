...
local Moran's I就是看cluster or disperse，if GDP is significantly higher than its neighbour, it might be marked as "high-high"

Local Moran's I 输入zi（就是标准化后的value比如GDP，**zi=（x-x拔）/S方差**）和listw就是weight
...
-   adaptive distance weight matrix. 这个就是固定neighbour数量，然后调distance
...
We do the same for latitude with one key difference. We access the second value per each centroid with \[\[2\]\]. 再捞latitude
